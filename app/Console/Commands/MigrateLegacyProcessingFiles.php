...
class MigrateLegacyProcessingFiles extends Command
{
    protected $signature = 'files:migrate-legacy-processing
        {--delete-public : Delete the public copy after verification (run with application writes stopped)}';

    protected $description = 'Copy referenced legacy opinion/result files to private storage';
    ...
    public function handle(): int
    {
        ...
        foreach (HoSoXuLy::query()->select(['maHSXL', 'duongdanfileykien', 'duongdanfileketqua'])->cursor() as $application) {
            foreach ([
                'ykien' => $application->duongdanfileykien,
                'ketqua' => $application->duongdanfileketqua,
            ] as $directory => $encodedReferences) {
                $references = json_decode((string) $encodedReferences, true);
                if (! is_array($references)) {
                    continue;
                }

                foreach ($references as $reference) {
                    $relativePath = $this->relativePath($reference, $directory);
                    ...
                    try {
                        if ($private->exists($relativePath)) {
                            $alreadyPrivate++;
                            if ($this->deletePublicCopy($public, $relativePath)) {
                                $deleted++;
                            }
    ...
    private function deletePublicCopy(FilesystemAdapter $public, string $relativePath): bool
    {
    ...
    private function relativePath(mixed $reference, string $directory): ?string
    {
        if (! is_string($reference) || ! str_starts_with($reference, 'storage/'.$directory.'/')) {
            return null;
        }
