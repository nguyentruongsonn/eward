...
function renderTrackingResult(app, fallbackCode) {
  ...
  const statusLabel = app.status?.label || 'Đang xử lý';
  const statusBadge = renderStatusBadge(statusId, statusLabel);

  const occurredAt = app.timeline?.[0]?.occurred_at
    ? new Date(app.timeline[0].occurred_at).toLocaleDateString('vi-VN')
    : '10/09/2026';

  const rows = [
    ['Mã hồ sơ tiếp nhận', app.application_code || fallbackCode],
    ['Tên thủ tục hành chính', app.procedure_name || 'Đăng ký kết hôn'],
    ['Cơ quan giải quyết', 'Ủy ban nhân dân cấp Xã / Phường'],
  ...
  return el('div', { class: 'card', style: 'padding: 2rem; max-width: 680px; background: #ffffff; border: 1px solid #d0d7de; border-radius: 4px;' }, [
    ...
    el('div', { style: 'display: flex; flex-direction: column; gap: 0.75rem; font-size: 13px;' }, rows.map(([label, val]) => el('div', {
      style: 'display: flex; justify-content: space-between; border-bottom: 1px solid #f1f5f9; padding-bottom: 0.5rem;',
    }, [
      el('span', { style: 'color: #475569; font-weight: 500;' }, label),
      el('strong', { class: 'numeric-data', style: 'color: #0f172a;' }, String(val)),
    ]))),
  ]);
}
